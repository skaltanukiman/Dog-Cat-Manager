...
test("設定画面は表示設定、お世話日設定の順で即時反映の注意を表示する", () => {
  ...
  assert.match(component, /<span className="mt-1 block w-full sm:max-w-md">[\s\S]*className="block min-h-11 w-full rounded-md/);
  assert.match(component, /切り替え時刻：\{formatMinutesAsTime\(savedMinutes\)\}/);
  assert.match(component, /<div className="mt-2 max-w-3xl space-y-1 text-sm leading-6 text-slate-600">[\s\S]*<p>設定した時刻になると、食事・水・散歩・猫トイレなどが新しいお世話日に切り替わります。<\/p>[\s\S]*<p>例：8:00に設定した場合、翌日の7:59までは同じお世話日として扱います。<\/p>/);
  assert.match(component, /<div className="space-y-1">[\s\S]*<p>変更内容は、保存後すぐに反映されます。<\/p>[\s\S]*<p>[\s\S]*お世話の表示が「実施済み」から「未実施」、または「未実施」から「実施済み」に変わる場合があります。[\s\S]*<\/p>[\s\S]*<p>既存記録の日付は変更されません。<\/p>/);
  assert.match(component, /<div className="mt-4 w-full max-w-4xl rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">/);

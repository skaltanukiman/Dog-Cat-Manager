...
| 現在の Household と権限 | `src/lib/authorization.ts`, `src/lib/auth-context.ts`, `src/app/actions/households.ts`, `src/components/household-switcher.tsx` | `OWNER` / `ADMIN` / `MEMBER` / `VIEWER` の閲覧・共有データ編集・招待・解除・権限変更を共通判定する。Dog-Cat専用の `dog_cat_manager_current_household` Cookie は所属確認後にのみ更新し、Hamster Managerの選択状態を読み書き・削除しない。 |
...
- **画面またはURL:** `/records`、認証付きPet思い出画像 `/api/pet-records/[id]/image`。現在HouseholdのPetを犬・猫のspecies付きで選択し、「管理終了したPetも含む」を明示的に切り替える。表示scopeは `pet`（選択中Pet）と `household`（共有グループ全体）で、不正値・未指定は `pet` とする。
- **主なコンポーネント:** `PetRecordCreateForms`、`PetRecordTimeline`、`MemoryPetSelector`、`PetThumbnail`、`RecordTimeInput`、`RecordKeywordInput`、`RecordImageField`、`MemoryTagInput`、`PaginationLayout`。健康・体調、通院、投薬、ワクチン、思い出の5フォームと1つの共通タイムラインを提供する。VIEWERと管理終了Petは閲覧・検索・絞り込み・ページ移動だけを許可する。
- **Server Action:** `src/app/actions/pet-*-records.ts` と `src/app/actions/pet-records.ts` にPet専用の種類固定CRUDを置く。全mutationはtransaction内で最新membership、Demo、PetのHousehold所属と管理状態を再確認し、古い`updatedAt`による上書きを拒否する。業務データ、Pet専用Activity、Household revisionを同一transactionで確定し、commit後に `source: "petRecord"` をpublishする。
- **データアクセス・Prismaモデル:** `getPetRecordsPageData`、`PetRecord`、`PetHealthRecordDetail`、`PetMedicalVisitDetail`、`PetMedicationRecordDetail`、`PetVaccinationRecordDetail`、`PetMemoryRecordDetail`。思い出は `PetMemoryRecordPet` で複数Petへ関連付け、`PetMemoryRecordImage` で最大1画像を扱う。記録日は`careDayStartMinutes`を使わない通常のJST暦日、任意時刻は0〜1439分のJST壁時計として保存する。一覧と件数はHousehold境界を含むDB queryで種類・日付・キーワード・`#タグ`・お気に入りを絞り、最新順に20件ずつ取得する。
- **画像・設定:** Pet思い出画像は旧Hamster画像と分離した `PET_RECORD_IMAGE_DIR`（Docker既定 `/app/uploads/pet-records`）のHousehold別非公開ディレクトリへUUID WebPで保存する。`src/lib/pet-record-image.ts` がpath traversal防止、変換、保存、読込、削除、DB失敗時の補償削除を担当する。
- **関連テスト:** `tests/records.test.ts`（5種類のvalidation、検索・scope・ページング、schema/migration、認可、画像分離・整合性、UI）、`tests/household-activity.test.ts`（Pet Records 15イベント）、`tests/household-delete.test.ts`（4画像領域のcleanup）、`tests/csv-and-realtime.test.ts`（revision・publish回帰）。
- **依存関係:** `SavedMemoryTag` と汎用画像処理は安全な範囲で再利用する。旧Hamster Recordsのモデル、Action、query、component、`RECORD_IMAGE_DIR`、画像APIは移行期間中DB・コード上に残す。投薬・ワクチン通知とDashboard表示は後続対象とする。

### 旧Hamster Records（移行期間中の保持対象）
...
- **保持対象:** 旧Hamster RecordsのAction・query・component・Prismaモデルを移行期間中の回帰互換として残す。表側の`/records`はPet版へ切り替え済みで、旧Hamster思い出画像API `/api/records/[id]/image` は既存データ向けに保持する。

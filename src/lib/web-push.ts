...
/** private keyを返さずに、VAPID一式が有効なときだけ公開鍵をクライアントへ渡す。 */
...
/** @throws VAPID環境変数が不足しているか形式が不正な場合。 */
...
/** OriginとHostの双方が存在し、同じoriginを指す変更リクエストだけを許可する。 */
...
/** Content-Lengthがないchunked requestも、UTF-8実本文を読んだ後に上限を再検証する。 */
export async function readJsonRequestWithinLimit(request: Request) {
  if (!requestBodyIsWithinLimit(request)) return { ok: false as const };
  try {

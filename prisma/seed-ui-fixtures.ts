...
async function createFixtures(target: Awaited<ReturnType<typeof findTargetUser>>, now: Date) {
  ...
  const pets: Prisma.PetCreateManyInput[] = [
    { id: petId("KOTARO"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "コタロウ", species: "DOG", breed: "柴犬", sex: "MALE", birthDate: parseDateInput(dateOffset(today, -365 * 5)), adoptionDate: parseDateInput(dateOffset(today, -365 * 4)), memo: "散歩とボール遊びが大好きです。", profileImageFileName: profileByPet.get(petId("KOTARO")), createdAt: createdAgo(now, 24 * 70) },
    { id: petId("HANA"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "ハナ", species: "DOG", breed: null, sex: "FEMALE", birthDate: parseDateInput(dateOffset(today, -365 * 3)), adoptionDate: null, memo: null, profileImageFileName: null, createdAt: createdAgo(now, 24 * 60) },
    { id: petId("LEO_LONG"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "レオナルド・わんぱく・ジュニア", species: "DOG", breed: "グレート・ピレニーズとゴールデン・レトリーバーのミックス", sex: "UNKNOWN", birthDate: null, adoptionDate: parseDateInput(dateOffset(today, -800)), memo: LONG_MEMO, profileImageFileName: profileByPet.get(petId("LEO_LONG")), createdAt: createdAgo(now, 24 * 50) },
    { id: petId("CHAKO_INACTIVE"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "チャコ", species: "DOG", breed: "ミニチュア・ダックスフンド", sex: "FEMALE", birthDate: parseDateInput(dateOffset(today, -365 * 12)), adoptionDate: null, memo: "過去の記録確認用に管理終了状態を維持しています。", isActive: false, profileImageFileName: null, createdAt: createdAgo(now, 24 * 40) },
    { id: petId("MIKE"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "ミケ", species: "CAT", breed: "三毛猫", sex: "FEMALE", birthDate: parseDateInput(dateOffset(today, -365 * 4)), adoptionDate: parseDateInput(dateOffset(today, -365 * 3)), memo: "窓辺がお気に入りです。", profileImageFileName: profileByPet.get(petId("MIKE")), createdAt: createdAgo(now, 24 * 30) },
    { id: petId("SORA"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "ソラ", species: "CAT", breed: null, sex: "MALE", birthDate: null, adoptionDate: null, memo: null, profileImageFileName: null, createdAt: createdAgo(now, 24 * 20) },
    { id: petId("MOMO"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "モモ", species: "CAT", breed: "ブリティッシュショートヘア", sex: "UNKNOWN", birthDate: parseDateInput(dateOffset(today, -540)), adoptionDate: parseDateInput(dateOffset(today, -500)), memo: "ごはんの時間になると小さな声で知らせます。", profileImageFileName: profileByPet.get(petId("MOMO")), createdAt: createdAgo(now, 24 * 10) },
    { id: petId("LUNA_INACTIVE"), householdId: UI_FIXTURE_HOUSEHOLD_ID, name: "ルナ", species: "CAT", breed: "ラグドール", sex: "UNKNOWN", birthDate: null, adoptionDate: parseDateInput(dateOffset(today, -1500)), memo: "管理終了・empty state確認用です。", isActive: false, profileImageFileName: null, createdAt: createdAgo(now, 24 * 5) }
  ];
